...
export class AuthService {
    ...
    static async signIn(email, password) {
        try {
            ...
            if (process.env.NODE_ENV === 'development') {
                ...
                const mockUser = mockCredentials.find(cred => cred.email === normalizedEmail && cred.password === password);
                if (mockUser) {
                    useNotificationStore.getState().addNotification('Logged in with development credentials', 'success');
                    return {
                        id: `mock-${Date.now()}`,
            ...
            for (let attempt = 0; attempt < this.MAX_RETRIES; attempt++) {
                try {
                    ...
                    if (profileError)
                        throw profileError;
                    useNotificationStore.getState().addNotification('Successfully signed in', 'success');
                    return profile;
                }
                catch (error) {
                    lastError = error;
    ...
    static async signUp(email, password, userData) {
        try {
            ...
            useNotificationStore.getState().addNotification('Account created successfully! Please check your email.', 'success');
        }
        catch (error) {
            this.handleAuthError(error);
            throw error;
    ...
    static async signOut() {
        try {
            ...
            if (error)
                throw error;
            useNotificationStore.getState().addNotification('Successfully signed out', 'success');
        }
        catch (error) {
            this.handleAuthError(error);
            throw error;
