...
export class BookingService {
    ...
    async getAvailability(date, professionalId) {
        try {
    ...
    async createBooking(bookingData) {
        try {
            ...
            const { data: booking, error: bookingError } = await supabase
                ...
                end_time: new Date(new Date(`${bookingData.date}T${bookingData.time}`).getTime() + 60 * 60 * 1000).toISOString(),
                status: 'scheduled',
            ...
            useNotificationStore.getState().addNotification('Booking confirmed! Check your email for details.', 'success');
            return booking;
        }
        catch (error) {
            console.error('Booking error:', error);
            useNotificationStore.getState().addNotification('Booking failed. Please try again.', 'error');
            throw error;
        }
    }
}
