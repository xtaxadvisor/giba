...
export function ThreadList() {
  ...
  if (error) {
    return <p className="text-red-500">Error loading threads: {error}</p>;
  }
      ...
      {threads.map((thread) => (
        ...
        >
          ...
          <div className="mt-4 flex items-center">
            ...
            />
            <span className="ml-2 text-sm text-gray-600">{thread.author.displayName}</span>
            {thread.author.location && (
              <span className="ml-2 text-sm text-gray-500">• {thread.author.location}</span>
            )}
          </div>
        </div>
      ))}
